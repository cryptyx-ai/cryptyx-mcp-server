...
Package: `@cryptyx/mcp-server@0.5.0` — published on npm via Trusted Publishing (OIDC).
...
```
670+ crypto metrics, 160 signals, 17 composites across 8 factor classes. Multi-factor backtesting, signal persistence, and regime analysis for trading agents.
```
...
• 670+ metrics across 8 factor classes (CORR, EFF, FLOW, FUT, OB, OPT, TR, VOL)
...
23 tools. The 6-step conviction loop: DISCOVER (top metrics by IC) → DEFINE (multi-factor thesis) → VALIDATE (backtest at 8 horizons) → SCAN (universe scan) → STORE (fork_signal) → EXECUTE (via OKX/Kraken).
...
```
get_market_snapshot, get_signal_triggers, get_signal_catalog, get_top_signals,
get_factor_scores, get_composite_rankings, get_market_pulse, get_price_history,
get_signal_explanation, search_assets, get_regime_analysis, backtest_signal,
analyze_metric, scan_metric_universe, get_asset_liquidity, get_live_prices,
get_featured_metrics, get_asset_top_predictors, fork_signal, simulate_signal,
analyze_metrics_composite, get_competition_leaderboard, get_competition_rounds
```
...
**Description:**
```
CRYPTYX — conviction engine for autonomous crypto trading agents. 23 tools covering 670+ metrics, 160 signals, 17 composites across 8 factor classes, multi-factor backtesting, universe scans, signal persistence, and regime analysis. Works alongside OKX and Kraken execution toolkits.
```
...
**Description:**
```
CRYPTYX provides institutional-grade crypto intelligence for AI agents — 670+ metrics across 8 factor classes, 160 signals, 17 IC-weighted composites, multi-factor hypothesis testing, signal backtesting, and universe scanning across 200+ assets. Designed to complement exchange execution toolkits (OKX, Kraken) with a conviction layer: the agent defines its own thesis from raw metrics, backtests it, and scans the universe for live matches.
```
...
Signal management: get_signal_triggers, get_top_signals, get_signal_catalog, get_signal_explanation, backtest_signal, fork_signal, simulate_signal
Metric slicer: get_featured_metrics, get_asset_top_predictors, analyze_metric, scan_metric_universe, analyze_metrics_composite
...
**Package description:**
```
CRYPTYX MCP — crypto conviction engine for AI agents. 670+ metrics, 160 signals, 17 composites, multi-factor backtesting, signal persistence, regime analysis.
```
...
- 23 tools covering the full conviction loop: discover → define → validate → scan → execute
...
**Package description:**
```
CRYPTYX Tool Spec — institutional crypto intelligence for AI agents. Wraps the CRYPTYX MCP server (npx @cryptyx/mcp-server) exposing 23 tools across market data, factor analysis, signal management, metric backtesting, and regime classification.
```
...
Metric Slicer (hero features):
  ...
  - get_asset_top_predictors   # per-asset predictive power rankings
...
Signal Intelligence:
  ...
  - get_top_signals            # curated top 10 by 7d IC
